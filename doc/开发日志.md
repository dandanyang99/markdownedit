...
### 14) 导出：HTML / PDF / DOCX（设置弹窗 + 进度反馈）

- 目标：提供“先可用”的导出能力，并有明确的导出设置入口与进度反馈。
- 实现：
  - “文件 → 导出...” 打开导出设置弹窗：格式（HTML/PDF/DOCX）、PDF 纸张/方向、是否嵌入本地图片
  - 导出进度覆盖层：展示阶段文案 + 进度条（渲染/生成/写入）
  - HTML：Markdown 渲染为单文件 HTML（内联基础 CSS），可选将本地相对路径图片嵌入为 data URL
  - PDF：HTML → pdfmake（MVP），支持 A4/Letter + 纵向/横向；可选嵌入本地图片
  - DOCX：Markdown 解析 → docx（MVP）；可选嵌入本地图片
- 文件：
  - `src/App.tsx`
  - `src/App.css`
  - `package.json` / `package-lock.json`
...
### 15) 导出排障：错误详情可见 + 内置日志面板

- 问题：导出 PDF/DOCX 失败时提示“查看控制台日志”，但桌面端用户未打开 DevTools 时无法看到 console 输出。
- 处理：
  - 导出失败时在导出弹窗内展示错误详情（可复制）
  - 设置菜单增加“查看日志/清空日志”，并在应用内捕获 console 日志，便于排障
- 文件：
  - `src/App.tsx`
  - `src/App.css`
  - `src-tauri/src/lib.rs`

### 16) 修复：导出 PDF/DOCX 调用写文件命令参数不匹配

- 现象：导出 PDF/DOCX 失败，错误为 `write_binary_file_base64 missing required key args`。
- 原因：Rust 命令签名采用单参数 struct（参数名为 `args`），而前端 `invoke` 传参为扁平对象（`{ path, contentBase64 }`），导致参数解析失败。
- 修复：调整 `write_binary_file_base64` Rust 命令签名为扁平参数（`path`, `contentBase64`），与前端一致。
- 文件：
  - `src-tauri/src/lib.rs`

### 17) 修复：侧边栏 Tab 消失 + PDF 导出中文乱码

- Bug 1：打开文件后切换到“大纲”，侧边栏顶部的“文件列表/大纲”Tab 看起来会“消失”。
  - 原因：侧边栏内容区域可滚动，Tab 在滚动容器内且会跟随滚动（切换内容时沿用原 scrollTop，视觉上像消失）。
  - 修复：将 Tab 从可滚动区域移出，固定在侧边栏顶部区域（不随内容滚动）。
- Bug 2：PDF 导出中文显示为乱码。
  - 原因：pdfmake 默认字体不包含中文字符。
  - 修复：内置并加载 CJK 字体（Noto Sans CJK SC），PDF 导出时注册为默认字体。
- 文件：
  - `src/App.css`
  - `src/App.tsx`
  - `public/fonts/NotoSansCJKsc-Regular.otf`

### 18) 发布：GitHub Releases 自动打包（Windows/macOS/Linux）

- 目标：将当前版本发布为 GitHub Release，提供可下载的安装包与源码。
- 实现：
  - 新增 GitHub Actions 工作流：push tag（`v*`）时自动构建并上传 Release 资源
  - 产物：Windows `.exe`（NSIS）/ Linux `.deb` + `.rpm` + `.AppImage` / macOS `.dmg`（具体取决于平台 bundler 可用性）
  - 源码：GitHub 自动附带 Source code (zip/tar.gz)
- 文件：
